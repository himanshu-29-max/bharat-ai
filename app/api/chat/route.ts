...
export async function POST(req: Request) {
  try {
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // 🎨 IMAGE GENERATION — Segmind Flux Schnell
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    if (mode === "imagine") {
      try {
        ...
        if (!segRes.ok) {
          ...
          // Fallback to HuggingFace if Segmind fails
          return await generateWithHuggingFace(cleanMsg, process.env.HUGGINGFACE_API_KEY?.trim() || "");
        }

        const segData = await segRes.json();
        const imgBase64 = segData.image;
        if (!imgBase64) return NextResponse.json({ reply: "Image nahi bani, dobara try karo!" });

        return NextResponse.json({ reply: "✅ Yeh rahi teri image!", generatedImage: `data:image/jpeg;base64,${imgBase64}` });
      } catch (e) {
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // 👁️ VISION / DOCUMENT ANALYSIS — Groq Vision
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    if (imageBase64) {
      try {
        ...
        if (!reply) {
          console.error("Vision model error:", JSON.stringify(visionData));
          return NextResponse.json({ reply: "Image analyze nahi ho paya. Koi aur image try karo!" });
        }
...
RULES:
- Search data mila hai toh USI se jawab de, kuch mat banao.
...
- Context yaad rakho — "ye/woh/iska" ka matlab pichli baat se samjho.
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// HuggingFace fallback for image gen
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
